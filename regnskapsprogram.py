...
def run_main_program(create_new_account, csv_transactions_file, year_to_track, account_filepath):
    ...
    if create_new_account:
        if not os.path.exists(os.path.dirname(account_filepath)):
            success = False
            message = 'Ugyldig fil: ' + str(account_filepath)
            title = 'Avbrutt'
            return success, message, title
    ...
    if header_line.find('Bokføringsdato') != -1:
        ...
        if header_line.find('Bokført') != -1:
            ...
            raise Exception('Invalid format on ' + csv_transactions_file)
    ...
    if not str(year_to_track).isnumeric() or int(year_to_track) < 1900 or int(year_to_track) > 2500:
        success = False
        message = 'Error: Årstall lagret i eksisterende regnskap (' + str(year_to_track) + ') er ugyldig'
        title = 'Avbrutt'
        return success, message, title
...
def main():
    ...
    user_interface.UI_master.run_GUI(default_create_new_account, default_year, default_new_transactions_file_dir, default_account_location, default_exsisting_form, run_main_program)
